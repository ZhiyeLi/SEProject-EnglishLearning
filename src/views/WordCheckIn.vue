<template>
  <div class="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div class="max-w-6xl mx-auto p-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div class="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-lg p-4 border border-emerald-200">
              <div class="text-sm text-gray-600 mb-1 flex items-center">
                <i class="fas fa-book mr-2 text-emerald-500"></i>当前词汇类型
              </div>
              <div class="text-2xl font-bold text-emerald-600">{{ currentType?.name }}</div>
              <div class="text-sm text-gray-600 mt-2">
                总词汇数：<span class="font-semibold">{{ currentType?.totalWords }}</span>
              </div>
            </div>
            ...
            <div class="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-lg p-4 border border-blue-200">
              <div class="text-sm text-gray-600 mb-1 flex items-center">
                <i class="fas fa-check-circle mr-2 text-blue-500"></i>已打卡数量
              </div>
              <div class="text-2xl font-bold text-blue-600">{{ currentProgress.passedCount }}</div>
              <div class="text-sm text-gray-600 mt-2">
            ...
            <!-- 进度条 -->
            <div class="bg-gradient-to-br from-purple-50 to-pink-50 rounded-lg p-4 border border-purple-200 md:col-span-2">
              <div class="text-sm text-gray-600 mb-2 flex items-center">
                <i class="fas fa-chart-line mr-2 text-purple-500"></i>学习进度
              </div>
              ...
              <div class="flex justify-between text-sm text-gray-600">
                <span>{{ currentProgress.passedCount }} / {{ currentType?.totalWords }}</span>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
          <div class="flex flex-wrap gap-3">
            ...
            >
              <i :class="[isPaused ? 'fas fa-play' : 'fas fa-pause', 'mr-2']"></i>
              {{ isPaused ? '继续打卡' : '暂停打卡' }}
            </button>
          </div>
        </div>
        ...
        <div v-if="currentPlan" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
          <h3 class="text-lg font-bold text-gray-800 mb-4 flex items-center">
            <i class="fas fa-clipboard-list text-emerald-500 mr-2"></i>当前打卡计划
          </h3>
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="bg-emerald-50 rounded-lg p-4 border border-emerald-200">
              <div class="text-sm text-gray-600 mb-1">每日打卡数</div>
              <div class="text-2xl font-bold text-emerald-600">{{ currentPlan.wordsPerDay }}</div>
              <div class="text-xs text-gray-600 mt-1">个单词/天</div>
            </div>
            <div class="bg-blue-50 rounded-lg p-4 border border-blue-200">
              <div class="text-sm text-gray-600 mb-1">剩余单词数</div>
              <div class="text-2xl font-bold text-blue-600">{{ currentPlan.remainingWords }}</div>
              <div class="text-xs text-gray-600 mt-1">个单词</div>
            </div>
            <div class="bg-purple-50 rounded-lg p-4 border border-purple-200">
              <div class="text-sm text-gray-600 mb-1">需要天数</div>
              <div class="text-2xl font-bold text-purple-600">{{ currentPlan.daysNeeded }}</div>
          ...
          <div class="mt-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
            <p class="text-sm text-gray-700">
              ...
              <span v-if="currentPlan.status === 'paused'" class="text-orange-600 font-semibold">
                打卡已暂停
              </span>
              <span v-else class="text-emerald-600 font-semibold">
                按照计划，您将在 <strong>{{ currentPlan.daysNeeded }}</strong> 天内完成 <strong>{{ currentPlan.remainingWords }}</strong> 个单词的打卡。
              </span>
            </p>
          </div>
        </div>
        ...
        <!-- 开始打卡按钮 -->
        <div v-if="currentProgress.passedCount < currentType?.totalWords" class="text-center">
          <button
            @click="startCheckIn"
          ...
          >
            ...
            {{ isPaused ? '已暂停 - 请先继续打卡' : currentPlan ? '开始打卡' : '请先制订打卡计划' }}
          </button>
          <p class="text-gray-600 text-sm mt-4">
            <i class="fas fa-lightbulb mr-1 text-yellow-500"></i>
            还剩 <span class="font-semibold text-emerald-600">{{ currentType?.totalWords - currentProgress.passedCount }}</span> 个单词未打卡
          </p>
        </div>
        ...
        <div v-else class="text-center py-12">
          <div class="inline-block">
            <div class="w-20 h-20 rounded-full bg-gradient-to-r from-emerald-400 to-teal-500 flex items-center justify-center mb-4 shadow-lg">
              <i class="fas fa-trophy text-white text-4xl"></i>
            </div>
            <h2 class="text-3xl font-bold text-gray-800 mb-2">恭喜！</h2>
            <p class="text-lg text-gray-600 mb-4">您已完成 {{ currentType?.name }} 的全部打卡</p>
            <button
              @click="switchWordType"
              class="px-6 py-2 bg-emerald-500 text-white rounded-lg hover:bg-emerald-600 transition-all"
            >
              <i class="fas fa-exchange-alt mr-2"></i>切换其他词汇类型
            </button>
          </div>
        </div>
      </div>
    ...
    <div v-if="showPlanModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8 animate-fadeIn">
        <h2 class="text-2xl font-bold text-gray-800 mb-1">
          <i class="fas fa-calendar-check text-emerald-500 mr-2"></i>制订打卡计划
        </h2>
        <p class="text-gray-600 mb-6">选择每天要打卡的单词数量（1-100）</p>

        <!-- 错误提示 -->
        <div v-if="planError" class="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg">
          <p class="text-red-700 text-sm flex items-center">
            <i class="fas fa-exclamation-circle mr-2"></i>{{ planError }}
          </p>
        </div>

        <!-- 输入框 -->
        <div class="mb-6">
          <label for="planInput" class="block text-sm font-semibold text-gray-700 mb-2">
            每天打卡数量（单词/天）
          </label>
          <input
            id="planInput"
        ...
        <!-- 计算结果预览 -->
        <div v-if="planWordsPerDay && planWordsPerDay >= 1 && planWordsPerDay <= 100" class="mb-6 p-4 bg-emerald-50 rounded-lg border-2 border-emerald-200">
          <p class="text-sm text-gray-700 mb-2">
            <span class="font-semibold">计划预览：</span>
          </p>
          <ul class="text-sm text-gray-700 space-y-1">
            <li>📌 每天打卡：<span class="font-semibold text-emerald-600">{{ planWordsPerDay }}</span> 个单词</li>
            <li>📌 剩余单词：<span class="font-semibold text-emerald-600">{{ currentType?.totalWords - currentProgress.passedCount }}</span> 个单词</li>
            <li>📌 需要天数：<span class="font-semibold text-emerald-600">{{ calculateDaysNeeded(planWordsPerDay) }}</span> 天</li>
          </ul>
        </div>
        ...
        <div class="flex gap-3">
          ...
          <button
            @click="confirmPlan"
            :disabled="!planWordsPerDay || planWordsPerDay < 1 || planWordsPerDay > 100"
          ...
          >
            <i class="fas fa-check mr-2"></i>确认计划
          </button>
        </div>
      </div>
    </div>
    ...
    <div v-if="showReviewModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-2xl shadow-2xl max-w-2xl w-full p-8 animate-fadeIn max-h-96 overflow-y-auto">
        <div class="flex items-center justify-between mb-6">
          <h2 class="text-2xl font-bold text-gray-800">
            <i class="fas fa-redo text-blue-500 mr-2"></i>复习已打卡单词
          </h2>
          <button
        ...
        <div class="space-y-3 mb-6">
          ...
          >
            <div>
              <p class="font-semibold text-gray-800">{{ getWordById(wordId)?.word }}</p>
              <p class="text-sm text-gray-600">{{ getWordById(wordId)?.translation }}</p>
            </div>
            <div class="flex items-center gap-2">
              <span class="text-xs bg-emerald-100 text-emerald-700 px-2 py-1 rounded">已打卡</span>
              <button
        ...
        <p class="text-sm text-gray-600 text-center">
          共 <span class="font-semibold">{{ currentProgress.passedWords.length }}</span> 个已打卡单词
        </p>
      </div>
    </div>
...
const progressPercentage = computed(() => {
  ...
  return Math.round((currentProgress.value.passedCount / currentType.value.totalWords) * 100)
})
...
function confirmPlan() {
  ...
  if (!planWordsPerDay.value || planWordsPerDay.value < 1 || planWordsPerDay.value > 100) {
    planError.value = '请输入1-100之间的数字'
    return
  }

  const plan = wordProgressManager.createPlan(currentType.value.id, planWordsPerDay.value)
...
function unmarkWordAsPassed(wordId) {
  ...
  currentProgress.value = wordProgressManager.getTypeProgress(currentType.value.id)
}
</script>
