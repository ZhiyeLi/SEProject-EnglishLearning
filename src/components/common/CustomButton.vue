<template>
  <button class="font-medium rounded-lg transition-all duration-200 flex items-center justify-center shadow-sm hover:shadow focus:outline-none focus:ring-2 focus:ring-offset-2" :class="[
    ...
    <i :class="['fas', icon, 'mr-2']" v-if="icon"></i>
    <slot>{{ text }}</slot>
  </button>
</template>
