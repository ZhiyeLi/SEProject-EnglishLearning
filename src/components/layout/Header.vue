...
const navLinks = ref([
  { key: 'home',text: '首页', url: '/', isActive: true },
  { key: 'course',text: '课程', url: '#', isActive: false },
  { key: 'question',text: '题库', url: '#', isActive: false },
  { key: 'schedule',text: '时间表', url: '#', isActive: false },
  { key: 'checkin',text: '单词打卡', url: '#', isActive: false },
  { key: 'ai',text: 'AI聊天', url: '/ai', isActive: false },
  { key: 'chat',text: '好友聊天', url: '/chat', isActive: false },
])

const navigate = (key) => {
  // If user clicked the chat link, use the router to go to the ChatPanel route
  if (key === 'chat') {
  ...
  // close mobile menu after navigating on mobile
  mobileMenuOpen.value = false
}
